...
class ServicesController extends Controller
{
    public function enroll(Request $request){
        ...
        $user = Biometric::where('user_id', $data['user_id'])->get();

        if($user < 1){
            $is_fraud = $this->is_duplicate($data->curr_finger_data);
            if($is_fraud == false){
                $curr_finger_data = $data->curr_finger_data;
                ...
                $verify = $this->verify($data['user_id'], $data['curr_finger_data']);
                if($verify == "match"){
